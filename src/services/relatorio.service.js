import { getTransactions } from '../models/transacao.model.js';
import { getBills } from '../models/conta.model.js';
import { getInvestments } from '../models/investimento.model.js';
import { formatCurrency, formatMonthLabel, previousMonth } from '../utils/auxiliares.js';
...
export function buildMonthStats(userId, month) {
    ...
    // Apenas investimentos iniciados NESTE mês contribuem para o saldo mensal
    const monthInvestments = investments.filter(i => (i.startDate || i.date || '').slice(0, 7) === month);
    const totalInvested = investments.reduce((s, inv) => {
        const allContribs = inv.contributions || [];
        const allContribTotal = allContribs.reduce((cs, c) => cs + c.amount, 0);
        // Valor original de criação (sem acumular aportes posteriores)
        const originalInitial = (inv.initialAmount || 0) - allContribTotal;
        // Novo investimento criado neste mês
        const isNewThisMonth = (inv.startDate || inv.date || '').slice(0, 7) === month;
        // Aportes feitos neste mês (em qualquer investimento)
        const monthContribs = allContribs
            .filter(c => (c.date || '').slice(0, 7) === month)
            .reduce((cs, c) => cs + c.amount, 0);
        return s + (isNewThisMonth ? originalInitial : 0) + monthContribs;
    }, 0);
    const totalOutflow = totalExpense + totalBills;
    // Investimentos são alocação positiva de capital (dinheiro que virou patrimônio)
